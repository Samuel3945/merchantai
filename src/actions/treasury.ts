...
// ── recordSelectedSupplierPaymentFromConsole ─────────────────────────────────
// Treasury-funded payment of CHOSEN invoices (full or partial each), from a
// cofre/banco. Settles exactly the payables the owner ticked in the modal (not a
// single amount auto-allocated oldest-first). ASSET debit, no P&L. Gated by
// requirePanelModule('cash').

export async function recordSelectedSupplierPaymentFromConsole(input: {
  supplierId: string;
  fromAccountId: string;
  selections: { payableId: string; amount: number | string }[];
  note?: string | null;
  ...
  if (!input.selections || input.selections.length === 0) {
    return { ok: false, error: 'Seleccioná al menos una factura' };
  }

  const selections = input.selections.map(s => ({
    payableId: s.payableId,
    amount: Number.parseFloat(toMoney(s.amount)),
  }));
  if (selections.some(s => !(s.amount > 0))) {
    return { ok: false, error: 'El monto de cada factura debe ser mayor a 0' };
  }

  const actor = await getActorName(userId);

  try {
    const result = await recordSelectedPayablesPayment(db, {
      organizationId: orgId,
    ...
    await logAction({
      organizationId: orgId,
      actor: { type: 'user', id: userId },
      action: 'treasury.supplier.settled_selected',
