...
// Generic .xlsx → header-keyed records parser, shared by every importer (products,
// suppliers, …). It mirrors the shape papaparse produces for CSV so callers feed
// both through the same `recordsToDrafts`. Runs server-side so the (heavy)
// spreadsheet parser never reaches the client bundle. The first sheet's first row
